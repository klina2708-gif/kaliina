...
function App(){
 ...
 const generate=()=>{if(status==='generating')return;timers.current.forEach(id=>clearTimeout(id));const final=makeResult();const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;if(reduced){setValues(final);setStatus('generated');return}setStatus('generating');setRolling(categories.map(([type])=>type));categories.forEach(([type],index)=>{const spin=setInterval(()=>setValues(old=>({...old,[type]:random(sources[type])})),52+index*4);timers.current.push(spin);const stop=setTimeout(()=>{clearInterval(spin);setValues(old=>({...old,[type]:final[type]}));setRolling(old=>old.filter(x=>x!==type));if(index===categories.length-1)setStatus('generated')},790+index*115);timers.current.push(stop)})};
 return <main>
