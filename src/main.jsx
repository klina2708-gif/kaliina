...
function RandomizerCell({index,type,label,value,generating}){const compact=['theme','name','mood'].includes(type)&&value?.name?.length>15;const animate=generating&&type!=='fonts';return <div className={`cell ${animate?'cell--rolling':''}`}><span className="cell__number">[{index+1}]</span><b className="cell__label">{label}</b><div className="cell__window"><div className={`cell__value ${compact?'cell__value--compact':''}`}><DisplayValue type={type} value={value}/></div>{animate&&<><div className="ghost ghost--one"><DisplayValue type={type} value={random(sources[type])}/></div><div className="ghost ghost--two"><DisplayValue type={type} value={random(sources[type])}/></div></>}</div></div>}
